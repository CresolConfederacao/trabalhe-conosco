...
dhtmlXTabBar.prototype = {
    ...
    setSkin: function (a) {
        ...
        if (b) {
            ...
            this._s.skin_line = -3;
            var d = this._s._rendered;
            if (d) for (var e = 0; e < d.length; e++) d[e].parentNode.removeChild(d[e]);
            var f = document.createElement("DIV");
            f.className = "dhx_tabbar_lineA";
            this._tabAll.appendChild(f);
            var g = document.createElement("DIV");
            g.className = "dhx_tabbar_lineB";
            this._tabAll.appendChild(g);
            var h = document.createElement("DIV");
            h.className = "dhx_tabbar_lineC";
            this._tabAll.appendChild(h);
            var i = document.createElement("DIV");
            i.className = "dhx_tabbar_lineD";
            this._tabAll.appendChild(i);
            this._getCoverLine();
            this._s._rendered = [f, g, h, i];
            if (this._s.expand) {
                this._conZone.style.borderWidth = "0px 0px 0px 0px";
                this._tabZone.firstChild.style.borderWidth = "0px 0px 0px 0px";
                h.style.borderWidth = "0px 0px 0px 0px";
                i.style.left = "0px";
                h.style.right = "0px";
                f.style.borderWidth = "0px 0px 0px 0px";
            ...
            var j = function () {
                    ...
                    b < 6 || (f.style[this._py] = parseInt(this._conZone.style[this._py]) - 3 + "px", f.style[this._dx] = b - 2 + "px", g.style[this._py] = parseInt(this._conZone.style[this._py]) - 3 + "px", g.style[this._dx] = b - (a ? 6 : 4) + (this._s.expand ? 2 : 0) + "px", h.style[this._dy] = parseInt(this._tabZone.style[this._dy]) - 3 + "px", this._lineA.style[this._dx] = b - 2 + "px")
                };
            j.call(this);
            var k = this._checkScroll;
            this._checkScroll = function () {
                j.apply(this, arguments);
                k.apply(this, arguments)
            };
            var l = this._scrollTo;
    ...
    _setRowSizes: function () {
        for (var a = 0; a < this._rows.length; a++) this._rows[a].style[this._dy] = this._s.line_height + "px", this._rows[a].style[this._py] = a * (this._s.line_height - (this._s.skin_line_x ? 4 : 2)) - (_isIE && !window.postMessage && this._s._bMode ? 0 : 0) + "px", this._rows[a].style.zIndex = 10 + a;
        this._setSizes()
    },
    _setTabSizes: function (a) {
        for (var b = this._a.offset, c = this._vMode ? this._pxc : this._px, d = 0; d < a.tabCount; d++) {
    ...
    addTab: function (a, b, c, d, e) {
        ...
        for (var e = e || 0, f = this._rows.length; f <= e; f++)
        this._createRow();
        var g = this._rows[e].tabCount;
        !d && d !== 0 && (d = g);
        var h = this._createTab(a, b, c == "*" ? 10 : c || this._s._tabSize, true);
        this._addTab(this._rows[e], h, c, d);
        this._tabs[a] = h;
        this.cells(a).hide();
        this._checkScroll()
    },
    addBtNewTab: function (a, b, c, d, e) {
        //!this.__skin && dhtmlx.skin && this.setSkin(dhtmlx.skin);
        for (var e = e || 0, f = this._rows.length; f <= e; f++)
        this._createRow();
        var g = this._rows[e].tabCount;
        !d && d !== 0 && (d = g);
        var h = this._createTab(a, b, c == "*" ? 10 : c || this._s._tabSize, false);
        this._addTab(this._rows[e], h, c, d);
        this._tabs[a] = h;
        this.cells(a).hide();
        this._checkScroll()
    ...
    _createTab: function (a, b, c, close) {
        var d = document.createElement("DIV");
        d.className = "dhx_tab_element dhx_tab_element_inactive";
        var e = "";
        if (close) {
        	e = "<span>" + b + "</span><div></div><div></div><div></div>";
            this._close && (e += "<IMG style='" + this._pxc + ":3px; " + this._py + (this.skin == "dhx_web" ? ":7" : ":3") + "px; position:absolute;z-index:11;' src='" + this.imgUrl + this.skin + "/close.png' >");
        } else {
        	e = "<div></div><div></div><div></div><div></div><IMG id='imgAdicionarAba' style='" + this._pxc + ":5px; "+ this._py + ":2px; position:absolute;z-index:11;' src='" + this.imgUrl + this.skin + "/icon_adicionar.png'>";
        }
        d.innerHTML = e;
        ...
        if (close) {
            if (this._close) {
                ...
                d.childNodes[4].onclick = function (a) {
                    ...
                    f.callEvent("onTabClose", [b]) && removerAbaColmeia(b)
                }
            }
        } else {
            var f = this;
    ...
    _img: function (a, b, c, d, e) {
        if (typeof a != "undefined") a.style.backgroundImage = "url(" + this.imgUrl + this.skin + "/" + this.skin + "_" + this._s.mode + ".gif)", a.style.backgroundPosition = this._get_img_pos(b), a.style[this._py] = "0px", c && (a.style[this._dx] = Math.max(d, 0) + "px", a.style[c] = (e || 0) + "px")
    },
    ...
    adjustTabSize: function (a, b) {
        ...
        if (!b) a.style.fontWeight = "bold", a.childNodes[3].style[this._dx] = a.style[this._dx] = "10px", b = a[this._s._vMode ? "scrollHeight" : "scrollWidth"] + 5 + (this._close ? 14 : 0), a.style.fontWeight = "";
    ...
    _showHover: function (a) {
    	if(a !=null && a.getAttribute("tab_id")=='idAbaAdicionarAba')
        	return;
        if (a != this._lastHower) {
            if (this._lastHower && this._lastHower != this._lastActive) {
                ...
                b.hover ? this._img_all(this._lastHower) : this._img(this._lastHower.childNodes[3], 5, this._px, parseInt(this._lastHower.style[this._dx]) - (b.left + b.right), b.left);
                this._lastHower = null
            }
            if (!a || !(a == this._lastActive || a._disabled)) if (this._lastHower = a) b = this._getSkin(a), a.className += " dhx_tab_hover", b.hover && (this._img(a.childNodes[1], 7, this._px, b.left), this._img(a.childNodes[2], 8, this._pxc, b.right)), this._img(a.childNodes[3], 0, this._px, parseInt(a.style[this._dx]) - (b.left + b.right), b.left)
        }
    },
    ...
    _onClickHandler: function (a) {
        ...
        if(c !=null && c.getAttribute("tab_id")=='idAbaAdicionarAba')
        	return;
    ...
    _setTabTop: function (a) {
        for (var b = this._rows.length - 1, c = 0; c < this._rows.length; c++) if (this._rows[c] == a.parentNode) {
            var d = this._rows[c];
    ...
    _getCoverLine: function () {
        if (!this._lineA) this._lineA = document.createElement("div"), this._lineA.className = "dhx_tablist_line", this._lineA.style[this._py] = this._s.line_height - 3 - (this._s.skin_line_x ? 1 : 0) + "px", this._lineA.style[this._dx] = "100%", this._lineA._index = 1;
    ...
    _parseXML: function (a, b, c, d, e) {
        ...
        for (var j = 0; j < h.length; j++) for (var k = e.doXPath("./tab", h[j]), l = 0; l < k.length; l++) {
            var p = k[l].getAttribute("width"),
            ...
            else for (var o = 0; o < k[l].childNodes.length; o++) {
                var n = k[l].childNodes[o];
    ...
    enableContentZone: function (a) {
        this._conZone.style.display = convertStringToBoolean(a) ? "" : "none";
        this._setSizes()
    },
    ...
    goToPrevTab: function (a) {
        do a = this.getNext(a || this._lastActive, "previousSibling");
        while (!this._setTabActive(a));
        return a
    },
    goToNextTab: function (a) {
        do { 
        	var oldA = a;
        	a = this.getNext(a || this._lastActive);
        	if(a !=null && a.getAttribute("tab_id")=='idAbaAdicionarAba') return this.goToPrevTab(oldA);
        } while (!this._setTabActive(a));
        return a
    },
    ...
    setLabel: function (a, b, c) {
        var d = b;
        if(b!=null && b.length > this._rows[0].tamanhoLabel){
        	d = b.substring(0,this._rows[0].tamanhoLabel)+'...';
        }
        this._tabs[a].firstChild.innerHTML = d;
        this._tabs[a].title = b;
        this.adjustTabSize(this._tabs[a], c);
        this._setTabSizes(this._tabs[a].parentNode);
        this._checkScroll();
    },
    aumentarTamanhoLabelAba: function (aumentar) {
    	this._rows[0].tamanhoLabel = 11;
    	if(aumentar){
    		this._rows[0].tamanhoLabel = 17;
    	}
    	var b = '';
    	var c = ''; 
        for (var a in this._tabs){
        	if(a!='idAbaAdicionarAba'){
		        b  = this._tabs[a].title;
		        if(b!=null && b.length > this._rows[0].tamanhoLabel){
		        	c = b.substring(0,this._rows[0].tamanhoLabel)+'...';
		        } else {
		        	c = b;
		        }
		        this._tabs[a].firstChild.innerHTML = c;
		        this.adjustTabSize(this._tabs[a]);
		        this._setTabSizes(this._tabs[a].parentNode);
	        }
        }
    },
    ...
    normalize: function (a, b) {
        ...
        for (var a = a || this._tabZone.offsetWidth, d = [], e = 0; e < this._rows.length; e++) for (var f = 0; f < this._rows[e].tabCount; f++) d.push(this._rows[e].removeChild(this._rows[e].childNodes[0]));
    ...
    showInnerScroll: function () {
        for (var a in this._tabs) if (this.cells(a).vs) {
...
if (!window.dhtmlXContainer) window.dhtmlXContainer = function (a) {
    ...
    this.obj._genStr = function (a) {
        for (var b = "", e = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789", f = 0; f < a; f++) b += e.charAt(Math.round(Math.random() * e.length));
        return b
    };
    ...
    this.obj.adjustContent = function (a, b, e, f, g) {
        ...
        if (f != !0 && this.dhxcont.offsetWidth > a.clientWidth) this.dhxcont.style.width = Math.max(0, a.clientWidth * 2 - this.dhxcont.offsetWidth) + "px";
        ...
        if (this._minDataSizeW != null && parseInt(this.dhxcont.style.width) < this._minDataSizeW) this.dhxcont.style.width = this._minDataSizeW + "px";
    ...
    this.obj._attachContent = function (a, d, e) {
        if (e !== !0) for (; b.dhxcont.mainCont.childNodes.length > 0;) b.dhxcont.mainCont.removeChild(b.dhxcont.mainCont.childNodes[0]);
        if (a == "url") {
        ...
        else if (a == "str") b.dhxcont._frame = null, b.dhxcont.mainCont.innerHTML = d
    };
    ...
    dhtmlx.extend_api("dhtmlXTabBar", {
        ...
        tabs: function (a) {
            for (var b = 0; b < a.length; b++) {
                var c = a[b];
                this.addTab(c.id, c.label, c.width, c.index, c.row);
